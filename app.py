...
def make_grayscale(input_file) :

    image_array = np.formstring(input_file, dtype='uint8')
    print("Image array :- ",image_array)

    decode_array_to_image = cv2.imdecode(image_array, cv2.IMREAD_UNCHANGED)
    print('Decode :- ', decode_array_to_image)

    converted_gray_image = cv2.cvtColor(decode_array_to_image, cv2.COLOR_BGR2GRAY)
